...
class location(object):
    def __init__(self, address):
        ...
        loc = Nominatim(user_agent="GetLoc")  # Create a Nominatim geocoder instance with a custom user-agent
        getLoc = loc.geocode(address)  # Use the geocoder to get the location details (latitude and longitude)
        
        # If a location is found, assign latitude and longitude attributes
        if getLoc:
            print(getLoc)  # Print the full geocoded location details (useful for debugging)
            self.latitude = getLoc.latitude  # Assign the latitude of the geocoded location
            self.longitude = getLoc.longitude  # Assign the longitude of the geocoded location
        else:
